...
public class FamilyTree
{
	...
	private void addLine(String line) throws TreeException {
		...
		if (colonIndex < 0) {
			throw new TreeException("Colon does not exist in line");
		}
		...
		if (root == null) {
			...
			if (parentNode == null) {
				throw new TreeException("Parent does not exist in tree.");
			}
	...
	TreeNode getMostRecentCommonAncestor(String name1, String name2) throws TreeException
	{
		...
		if (node1 == null)
			throw new TreeException("first node does not exist");
		TreeNode node2 = root.getNodeWithName(name2);		// node whose name is name2
		if (node2 == null)
			throw new TreeException("second node does not exist");
